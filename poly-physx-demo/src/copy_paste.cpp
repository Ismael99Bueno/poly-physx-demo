...
void copy_paste::erase_group(const std::string &name)
{
    m_groups.erase(name);
}
...
static bool contains_ids(const std::unordered_map<ppx::uuid, ppx::entity2D::specs> &entities, const ppx::joint2D &joint)
{
    return entities.find(joint.e1().id()) != entities.end() && entities.find(joint.e2().id()) != entities.end();
}
...
void copy_paste::copy(group &g)
{
    ...
    for (const ppx::spring2D &sp : papp.engine().springs())
        if (contains_ids(g.entities, sp))
            g.springs.emplace_back(ppx::spring2D::specs::from_spring(sp), sp.e1().id(), sp.e2().id());
    ...
    for (const auto &ctr : papp.engine().compeller().constraints())
    {
        ...
        if (contains_ids(g.entities, *rb))
            g.rbars.emplace_back(ppx::rigid_bar2D::specs::from_rigid_bar(*rb), rb->e1().id(), rb->e2().id());
    }
}

template <typename T, typename F>
static void add_joints(const std::unordered_map<ppx::uuid, ppx::entity2D_ptr> &added, std::vector<T> &idjoint,
                       F add_fun)
{
...
void copy_paste::paste()
{
    ...
    add_joints(added_entities, m_copy.springs,
               [&papp](const ppx::spring2D::specs &spc) { papp.engine().add_spring(spc); });
                           ...
                           const std::vector<T> &idjoints, sf::Color col, const glm::vec2 &ref_pos, F draw_fun)
{
    ...
    {
        const ppx::entity2D::specs &e1 = entities.at(idjoint.id1), &e2 = entities.at(idjoint.id2);
        col.a = 120;
...
void copy_paste::preview()
{
    ...
    preview_joints(
        m_copy.entities, m_copy.springs, papp.springs_color(), m_copy.ref_pos,
        [&papp](const glm::vec2 &p1, const glm::vec2 &p2, const sf::Color &c) { papp.draw_spring(p1, p2, c); });
    preview_joints(
        m_copy.entities, m_copy.rbars, papp.rigid_bars_color(), m_copy.ref_pos,
        [&papp](const glm::vec2 &p1, const glm::vec2 &p2, const sf::Color &c) { papp.draw_rigid_bar(p1, p2, c); });
}
...
const std::map<std::string, copy_paste::group> &copy_paste::groups() const
{
    return m_groups;
}
const copy_paste::group &copy_paste::current_group() const
{
    return m_copy;
}
...
template <typename T> static void emit_idjoint(YAML::Emitter &out, const T &idjoint)
{
    out << YAML::BeginMap;
...
template <typename T> YAML::Node encode(const T &idjoint)
{
    YAML::Node node;
...
template <typename T> T decode(const YAML::Node &node)
{
    T idjoint;
