...
void demo_app::on_end()
{
    write_save(LAST_SAVE);
}
...
bool demo_app::read_save(const std::string &filename)
{
    return deserialize(SAVES_DIR + filename);
}
...
bool demo_app::read_example(const std::string &filepath)
{
    return deserialize(EXAMPLES_DIR + filepath);
}
...
void demo_app::remove_selected() // TODO: Que remove selected se llame solo con backspace una vez se cancele tb con clic
                                 // der
{
...
const std::string &demo_app::session() const
{
    return m_session;
}
...
bool demo_app::has_session() const
{
