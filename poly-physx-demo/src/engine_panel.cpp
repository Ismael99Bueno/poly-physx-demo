...
void engine_panel::render_sliders()
{
    ...
    {
        const rk::integrator &integ = papp.engine().integrator();
        const int mm = (int)(1.f / integ.max_dt()), mx = (int)(1.f / integ.min_dt());
...
{
    ImGui::PushItemWidth(150);
    static const char *methods[] = {"RK1", "RK2", "RK4", "RK38", "RKF12", "RKF45", "RKFCK45", "RKF78"};
...
void engine_panel::draw_quad_tree(const ppx::quad_tree2D &qt) const
{
    ...
    {
        const glm::vec2 &mm = qt.aabb().min(), &mx = qt.aabb().max();
        prm::flat_line_strip fls({glm::vec2(mm.x, mx.y) * PPX_WORLD_TO_PIXEL, mx * PPX_WORLD_TO_PIXEL,
                                  glm::vec2(mx.x, mm.y) * PPX_WORLD_TO_PIXEL, mm * PPX_WORLD_TO_PIXEL,
...
void engine_panel::draw_bounding_boxes() const
{
    for (const ppx::entity2D &e : demo_app::get().engine().entities())
    {
        ...
        const glm::vec2 &mm = bb.min(), &mx = bb.max();
        prm::flat_line_strip fls({glm::vec2(mm.x, mx.y) * PPX_WORLD_TO_PIXEL, mx * PPX_WORLD_TO_PIXEL,
                                  glm::vec2(mx.x, mm.y) * PPX_WORLD_TO_PIXEL, mm * PPX_WORLD_TO_PIXEL,
                                  glm::vec2(mm.x, mx.y) * PPX_WORLD_TO_PIXEL});
        demo_app::get().draw(fls);
