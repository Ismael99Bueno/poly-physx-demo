...
#include "ppx-demo/2D/actions/contraption_tab.hpp"
#include "ppx-demo/2D/app/demo_app.hpp"
#include "ppx-demo/2D/actions/body_tab.hpp"
#include "ppx-demo/2D/actions/joints_tab.hpp"
...
template <typename Line> void contraption_tab::create_preview_objects(const std::uint32_t segments)
{
    ...
    for (std::size_t i = 0; i < segments + 2; i++)
    {
        ...
        if (i > 0)
            m_preview_lines.push_back(kit::make_scope<Line>(lynx::color{*m_app->style.joint_color, 120u}));
    }
}
