...
float gravity::potential_energy(const ppx::entity2D &e) const
{
    return -e.mass() * p_mag * e.pos().y;
}
...
float gravitational::potential_energy_pair(const ppx::entity2D &e1, const ppx::entity2D &e2) const
{
    const float cte = -p_mag * e1.mass() * e2.mass();
...
{
    const float cte = p_mag * e1.charge() * e2.charge(), dist = glm::distance(e1.pos(), e2.pos());
    float den = dist;
...
float electrical::potential_energy_pair(const ppx::entity2D &e1, const ppx::entity2D &e2) const
{
    const float cte = p_mag * e1.charge() * e2.charge(), dist = glm::distance(e1.pos(), e2.pos());
    if (p_exp > 1)
    {
...
float exponential::potential_energy_pair(const ppx::entity2D &e1, const ppx::entity2D &e2) const
{
    const float cte = p_mag * e1.charge() * e2.charge(), dist = glm::distance(e1.pos(), e2.pos());
