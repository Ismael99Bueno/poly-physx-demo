...
void actions_panel::on_render()
{
    ...
    if (enabled)
        ImGui::HelpMarker(
            "The Actions panel allows you to make high level changes to the simulation, such as adding, removing and "
            "grabbing entities, modify their properties or attach springs and rigid bars to them.");
...
void actions_panel::update_current_action()
{
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::F))
        ...
        m_action = ATTACH;
    else if (sf::Keyboard::isKeyPressed(sf::Keyboard::LShift) || sf::Keyboard::isKeyPressed(sf::Keyboard::LControl))
        m_action = SELECT;
    else
        m_action = ADD;
}
