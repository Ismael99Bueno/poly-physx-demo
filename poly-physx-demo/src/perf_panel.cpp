...
void perf_panel::render_time_plot(const float last_physics, const float last_drawing) const
{
    ...
    if (ImPlot::BeginPlot("##Performance", ImVec2(-1, PLOT_HEIGHT), ImPlotFlags_NoMouseText))
    {
        ...
        ImPlot::PlotLine("Physics", &phys.data()->x, &phys.data()->y, (int)phys.size(), 0, (int)offset,
                         2 * sizeof(float));
        ImPlot::SetNextFillStyle(IMPLOT_AUTO_COL, .5f);
        ImPlot::PlotLine("Drawing", &draw.data()->x, &draw.data()->y, (int)draw.size(), 0, (int)offset,
                         2 * sizeof(float));
        ImPlot::SetNextFillStyle(IMPLOT_AUTO_COL, .5f);
...
void perf_panel::render_hierarchy(const perf::profile_stats &stats, const float conversion, const char *unit,
                                  int &call) const
{
    if (ImGui::TreeNode((void *)(intptr_t)call, "%s (%.2f %s, %.2f%%)", stats.name(),
                        stats.duration_over_calls() * conversion, unit, stats.relative_percent() * 100.0))
    {
        if (ImGui::CollapsingHeader("Details"))
        {
            ImGui::Text("Duration per execution: %f %s", stats.duration_per_call() * conversion, unit);
            ImGui::Text("Overall performance impact: %.2f %s (%.2f%%)",
                        stats.duration_per_call() * stats.total_calls() * conversion, unit,
...
{
    render_smooth_factor();
    const sf::Time &physics = demo_app::get().phys_time(), &drawing = demo_app::get().draw_time();
    ...
    if (hovered || ImGui::IsItemHovered())
        ImGui::SetTooltip(
            "The time it took for the different project components (physics/drawing/total) to render the last frame.");
