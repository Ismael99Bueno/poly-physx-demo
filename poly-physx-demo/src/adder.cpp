...
void adder::add_soft_body()
{
    ...
    const std::size_t per_iter = p_add_specs.entities_between_vertices + 1, entity_count = poly.size() * per_iter;
    ...
    for (std::size_t i = 0; i < poly.size(); i++)
    {
        ...
        for (std::size_t j = 0; j < per_iter; j++)
        {
            ...
            ppx::entity2D::specs spcs = {m_start_pos + pos,
                                         gen_specs.kinematic ? vel : glm::vec2(0.f),
                                         0.f,
                                         0.f,
                                         gen_specs.mass / entity_count,
                                         gen_specs.charge / entity_count,
                                         {},
                                         p_add_specs.sb_radius,
                                         gen_specs.kinematic,
                                         ppx::entity2D::CIRCLE};
    ...
    for (std::size_t i = 0; i < added.size(); i++)
        for (std::size_t j = i + 1; j < added.size(); j++)
        {
            ...
            papp.engine().add_spring(e1, e2, p_add_specs.sb_stiffness / spring_count,
                                     p_add_specs.sb_dampening / spring_count, glm::distance(e1->pos(), e2->pos()));
        }
}
...
void adder::load_template()
{
    load_template(p_add_specs.name);
}
void adder::erase_template()
{
    erase_template(p_add_specs.name);
}
...
bool adder::has_saved_entity() const
{
    return !p_add_specs.name.empty();
}
...
void adder::draw_velocity_arrow() const
{
    ...
    const glm::vec2 start = m_start_pos * PPX_WORLD_TO_PIXEL,
                        ...
                        (glm::length(vel) < max_arrow_length ? (m_start_pos + vel)
                                                             : (m_start_pos + glm::normalize(vel) * max_arrow_length)) *
                        PPX_WORLD_TO_PIXEL,
                    segment = start - end;
